...
Examples:
  ...
  # Test on MULTIPLE datasets (combined automatically) using --test-sources
  python code/test_inference/inference_gradcam.py --checkpoint-dir model_checkpoints \\
      --test-sources \\
          /path/to/images1 /path/to/data1.csv \\
          /path/to/images2 /path/to/data2.csv \\
          /path/to/images3 /path/to/data3.csv
  
  # Alternative: specify multiple CSVs and roots separately
  python code/test_inference/inference_gradcam.py --checkpoint-dir model_checkpoints \\
      --test-csv /path/to/data1.csv /path/to/data2.csv /path/to/data3.csv \\
      --test-root /path/to/folder1 /path/to/folder2 /path/to/folder3
  ...
  python code/test_inference/inference_gradcam.py
    ...
    # Test data options
    test_data_group = parser.add_argument_group('Test Data Options')
    test_data_group.add_argument(
        '--test-sources',
        nargs='+',
        metavar=('ROOT_DIR', 'CSV_FILE'),
        help='Pairs of root_dir and csv_file (e.g., /path/to/imgs1 /path/to/data1.csv /path/to/imgs2 /path/to/data2.csv)'
    )
    test_data_group.add_argument(
        '--test-csv',
        nargs='+',
        type=str,
        help='Path(s) to test CSV file(s) - can specify multiple (must match --test-root count)'
    )
    test_data_group.add_argument(
        '--test-root',
        nargs='+',
        type=str,
        help='Root director(ies) for test images - can specify multiple (must match --test-csv count)'
    )
    
    return parser.parse_args()


def parse_test_sources(args) -> Tuple[List[str], List[str]]:
    """
    Parse and validate test data sources from command-line arguments.
    Returns: (csv_paths, root_dirs) as two lists of equal length.
    """
    csv_paths = []
    root_dirs = []
    
    # Priority 1: --test-sources (paired format)
    if args.test_sources:
        if len(args.test_sources) % 2 != 0:
            print("ERROR: --test-sources requires pairs of ROOT_DIR CSV_FILE")
            print(f"Got {len(args.test_sources)} arguments (expected even number)")
            sys.exit(1)
        
        # Parse pairs: root_dir1, csv1, root_dir2, csv2, ...
        for i in range(0, len(args.test_sources), 2):
            root_dir = args.test_sources[i]
            csv_file = args.test_sources[i + 1]
            
            if not Path(root_dir).exists():
                print(f"WARNING: Test root directory not found: {root_dir}")
            if not Path(csv_file).exists():
                print(f"WARNING: Test CSV file not found: {csv_file}")
            
            root_dirs.append(root_dir)
            csv_paths.append(csv_file)
        
        print(f"✓ Loaded {len(csv_paths)} test source(s) from --test-sources")
        for csv, root in zip(csv_paths, root_dirs):
            print(f"  - CSV: {csv}")
            print(f"    Root: {root}")
    
    # Priority 2: --test-csv and --test-root (separate lists)
    elif args.test_csv and args.test_root:
        csv_paths = args.test_csv if isinstance(args.test_csv, list) else [args.test_csv]
        root_dirs = args.test_root if isinstance(args.test_root, list) else [args.test_root]
        
        if len(csv_paths) != len(root_dirs):
            print(f"ERROR: Number of --test-csv ({len(csv_paths)}) must match --test-root ({len(root_dirs)})")
            sys.exit(1)
        
        print(f"✓ Loaded {len(csv_paths)} test source(s) from --test-csv and --test-root")
        for i, (csv, root) in enumerate(zip(csv_paths, root_dirs), 1):
            print(f"  [{i}] CSV: {csv}")
            print(f"      Root: {root}")
    
    # Priority 3: Individual --test-csv or --test-root (legacy single source)
    elif args.test_csv or args.test_root:
        print("ERROR: Both --test-csv and --test-root must be specified together")
        sys.exit(1)
    
    return csv_paths, root_dirs
...
def main():
    ...
    # --- NEW: Parse test sources from command-line or config ---
    csv_paths_override, root_dirs_override = parse_test_sources(args)
    
    if csv_paths_override and root_dirs_override:
        # Use command-line specified sources
        test_csv_paths = csv_paths_override
        test_root_dirs = root_dirs_override
    else:
        # Use config.py sources
        test_csv_paths = inference_config.test_csv_paths
        test_root_dirs = inference_config.test_root_dirs
        print(f"Using test data from config.py ({len(test_csv_paths)} source(s))")
    
    all_dfs = []
    for i, (csv_path, root_dir) in enumerate(zip(test_csv_paths, test_root_dirs), 1):
        ...
        all_dfs.append(df)
        print(f"  [{i}] Loaded {len(df)} samples from {Path(csv_path).name}")
    ...
    print(f"\n✓ Combined total: {len(test_data)} test samples from {len(all_dfs)} source(s)")
